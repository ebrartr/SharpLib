...
namespace SharpLib.Extensions.File
{
    public static class FileUploadExtensions
    {
        ...
        /// <returns></returns>
        public static ResponseModel<List<SavedFileVM>> Save(this IFormFileCollection formCollection, FileUploadVM options = null)
        {
            if (options == null)
            ...
            if (!validCheck.ProcessStatus)
                return new ResponseModel<List<SavedFileVM>> { ProcessStatus = false, Message = validCheck.Message };
            ...
            if (savedResult.SavedFileList.Any())
                return new ResponseModel<List<SavedFileVM>> { ProcessStatus = true, Result = savedResult.SavedFileList };

            return new ResponseModel<List<SavedFileVM>> { ProcessStatus = false, Message = options.FileDidNotSaveMessage };
        }
    }
}
